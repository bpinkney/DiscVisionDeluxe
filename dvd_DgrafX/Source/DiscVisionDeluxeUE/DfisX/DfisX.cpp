...
{
    if (basic_console_logging) std::cout << "\n This is a simulated throw using a " << global_active_throw.disc_object.mold_name;
...
//step_Dcollision (active_throw, step_time);
  ...
  propagate (stepper_active_throw, step_time); 

  //temporary ground collision detection 
  if (stepper_active_throw.current_disc_state.disc_location[2] <= 0) 
  {
    finish_throw        (stepper_active_throw);
  }
    //in case of a hang
  else if (stepper_active_throw.current_disc_state.forces_state.step_count > 10000)
  {
    std::cout << "The throw simulation aborted due to reaching maximum steps (10000)\n";
    stepper_active_throw.current_disc_state.sim_state = SIM_STATE_STOPPED;
  }
...

{
    ...
    double test_time_aloft = global_active_throw.disc_state_array.size() * step_time_global;
    std::cout << "\n         The throw spent " << test_time_aloft << "s in the air";
    Eigen::Vector3d throw_distance_vector = global_active_throw.disc_state_array[0].disc_location - global_active_throw.disc_state_array[global_active_throw.disc_state_array.size()-1].disc_location;
    ...
    Eigen::Vector3d throw_velocity_vector = global_active_throw.disc_state_array[0].disc_velocity;
    ...
    std::cout << std::setprecision(3) << "The throw simulated " << global_active_throw.current_disc_state.forces_state.step_count << " steps before ending normally.\n";
    global_active_throw.current_disc_state.sim_state = SIM_STATE_STOPPED;
...
{
  if (global_active_throw.current_disc_state.sim_state == SIM_STATE_STOPPED) return true;
  else                                        return false;
}
...
//create the starting d state
  global_active_throw.current_disc_state = {thrown_disc_position,thrown_disc_velocity,thrown_disc_orientation,thrown_disc_rotation};
  global_active_throw.current_disc_state.sim_state = SIM_STATE_STARTED;  
  global_active_throw.current_disc_state.forces_state = {};
  global_active_throw.current_disc_state.forces_state.angular_velocity = thrown_disc_radians_per_second;
