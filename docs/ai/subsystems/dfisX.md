# DfisX Physics Engine — Reference
...
| File | Purpose |
|---|---|
| DfisXStructs.cs | DiscState, ForcesState, ThrowContainer, DiscModelData, AeroDebugSettings. No UnityEngine. GaussState field on ThrowContainer. |
...
### ThrowContainer
Managed class. Owns NativeArray<DiscState> discStateArray (resized to MAX_STEPS). Holds AeroDebugSettings and GaussState. FlightStats populated by BuildStats() in DiscFlightSimulator.
...
Unity Y-up conversion at render time via ToUnitySpace() / ToUnityRotation() in DiscFlightSimulator.

## Simulation Step
DiscFlightSimulator.StepForUnityFrame() advances sim by elapsed Unity delta time. Internally sub-steps at 1kHz (dtS = 0.001). MAX_STEPS = 30,000 (~30s). Calls Daero → Dgyro → Dpropagate each sub-step.

## Pending: Burst Migration (INF-4)
1. Replace System.Random in GaussState (Daero.cs) with Unity.Mathematics.Random
2. Add [BurstCompile] attributes
