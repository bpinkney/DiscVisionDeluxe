...
void new_throw (int disc_mold_enum,Eigen::Vector3d thrown_disc_position,Eigen::Vector3d thrown_disc_velocity, double thrown_disc_roll, double thrown_disc_pitch, double thrown_disc_radians_per_second, double thrown_disc_wobble)
...
//create the starting d state
  ...
  d_array.clear ();
  //active_throw.disc_state_array.begin ();
...
//step_Dcollision (active_throw, step_time);
  ...
  //temporary ground collision detection 
    ...
    {
      std::cout << "\nThe throw simulation aborted due to reaching maximum steps (1000)";
      d_state.sim_state = SIM_STATE_STOPPED;
    }
